...
WHERE (users.id = ANY ($1::uuid[]))
  AND (users.deleted_at IS NULL OR $2::boolean = TRUE)
ORDER BY users.created_at DESC
`

type FindUsersByIDsParams struct {
	Ids            []uuid.UUID
	IncludeDeleted bool
}
...
func (q *Queries) FindUsersByIDs(ctx context.Context, arg FindUsersByIDsParams) ([]FindUsersByIDsRow, error) {
	rows, err := q.db.QueryContext(ctx, findUsersByIDs, pq.Array(arg.Ids), arg.IncludeDeleted)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []FindUsersByIDsRow
	for rows.Next() {
		var i FindUsersByIDsRow
		if err := rows.Scan(&i.ID, &i.Nickname, &i.ProfileImageUrl); err != nil {
			return nil, err
		}
		items = append(items, i)
